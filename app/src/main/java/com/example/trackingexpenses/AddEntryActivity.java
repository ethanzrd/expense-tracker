...
public class AddEntryActivity extends AppCompatActivity {
    ...
    private String currentType = "income"; // Default type
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if (snapshot.exists()) {
                            ...
                            // Initialize spinner with income categories (default selection)
                            updateCategorySpinner(incomeCategories);
                        } else {
                    ...
                    public void onCancelled(@NonNull DatabaseError error) {
                        progressBar.setVisibility(View.GONE);
                        Toast.makeText(AddEntryActivity.this, "שגיאה בטעינת קטגוריות", Toast.LENGTH_SHORT).show();
                    }
                });
    }
                ...
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            ...
                            Toast.makeText(AddEntryActivity.this, "שגיאה ביצירת קטגוריות ברירת מחדל", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
    ...
    private void saveEntry() {
        ...
        // Save to Firebase
        String entryId = mDatabase.child("entries").child(currentUserId).push().getKey();
        mDatabase.child("entries").child(currentUserId).child(entryId).setValue(entryData)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        ...
                        if (task.isSuccessful()) {
                            Toast.makeText(AddEntryActivity.this, "הרשומה נשמרה בהצלחה", Toast.LENGTH_SHORT).show();
                            finish(); // Return to previous screen
                        } else {
                            Toast.makeText(AddEntryActivity.this, "שגיאה בשמירת הרשומה", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
